<template>
  <div class="settings bg-gray-800 text-white p-6 rounded-lg shadow-lg w-full max-w-2xl mx-auto">
    ...
    <div v-for="setting in settings" :key="setting.setting_id" class="mt-4">
      ...
      <div v-else-if="isSuccess && activeSetting === setting.setting_id" class="text-green-500 mb-2">Camera status updated
        successfully!</div>
...
const { data: settings } = await useFetch<Settings[]>('http://main.brazilsouth.cloudapp.azure.com:8000/settings')

const isLoading = ref(false);
const isSuccess = ref(false);
const activeSetting = ref<number | null>(null);

const toggleCamera = async (setting: Settings) => {
